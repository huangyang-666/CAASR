...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_dir', type=str, default='__assets__/lr_inputs',
                        help="Can be either single image input or a folder input")
    parser.add_argument('--model', type=str, default='GRL',
                        help=" 'GRL' || 'RRDB' (for ESRNET & ESRGAN) || 'CUNET' (for Real-ESRGAN) ")
    parser.add_argument('--scale', type=int, default=4, help="Up scaler factor")
    parser.add_argument('--weight_path', type=str, default='pretrained/4x_APISR_GRL_GAN_generator.pth',
                        help="Weight path directory, usually under saved_models folder")
    parser.add_argument('--store_dir', type=str, default='sample_outputs',
                        help="The folder to store the super-resolved images")
    # Currently, this is only supported in RRDB, there is some bug with GRL model
    parser.add_argument('--float16_inference', type=bool, default=False,
                        help="The folder to store the super-resolved images")
    ...
    generator = generator.to(dtype=weight_dtype)  # float16/32
    ...
    if os.path.isdir(store_dir):  # If the input is a directory, we will iterate it
        for _ in os.listdir(input_dir):
            cur_path = os.path.join(input_dir, _)
            temp1 = os.path.join(store_dir, _)
            os.makedirs(temp1, exist_ok=True)
            input_dir_1 = cur_path
            store_dir_1 = temp1
            for filename in sorted(os.listdir(input_dir_1)):
                input_path = os.path.join(input_dir_1, filename)
                output_path = os.path.join(store_dir_1, filename)
