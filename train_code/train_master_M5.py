...
def process_output(output, weight):
    # 对输出进行池化操作
    out = torch.nn.functional.adaptive_avg_pool2d(output, 1)
    out = out.reshape(out.shape[0], out.shape[1], -1).permute(0, 2, 1)
    total_l_reg, total_n_reg = torch.tensor(0).to(out), torch.tensor(0).to(out)
    ...
    for u, v in zip(x1, x2):
        ...
        l_reg += weight * reg(u, v)
        n_reg += weight * reg(a, b) + wave_similarity
    total_l_reg += l_reg
    total_n_reg += n_reg
    return total_l_reg, total_n_reg
...
class train_master(object):
    ...
    def single_iteration(self, imgs_lr, imgs_lr_reg, imgs_degrade_hr, imgs_hr, epoch, idx):
        ...
        with torch.cuda.amp.autocast():
            ...
            gen_hr, self.net_g_out1, self.net_g_out2, self.net_g_out3 = self.generator(imgs_lr)
            if idx % 700 == 0:
                save_image(gen_hr, f'check_M5_S2/epoch_{epoch}_genhr.png')
                save_image(imgs_lr, f'check_M5_S2/epoch_{epoch}_imglr.png')
                save_image(imgs_degrade_hr, f'check_M5_S2/epoch_{epoch}_imgs_degrade_hr.png')
                # save_image(imgs_hr, f'check_M5_S1/epoch_{epoch}_imgs_hr.png')
            ...
            # reg_loss
            # print(f"gen_hr.shape---->{gen_hr.shape}")
            # print(f"self.net_g_out.shape---->{self.net_g_out.shape}")
            # 分别处理每个网络输出
            out1_l_reg, out1_n_reg = process_output(self.net_g_out1, 2)
            out2_l_reg, out2_n_reg = process_output(self.net_g_out2, 4)
            out3_l_reg, out3_n_reg = process_output(self.net_g_out3, 8)
            l_reg = out1_l_reg + out2_l_reg + out3_l_reg
            n_reg = out1_n_reg + out2_n_reg + out3_n_reg
            self.generator_loss = self.generator_loss + l_reg
            self.generator_loss = self.generator_loss + n_reg
