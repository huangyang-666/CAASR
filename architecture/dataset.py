...
class ImageDataset(Dataset):
    @torch.no_grad()
    def __init__(self, train_lr_paths, degrade_hr_paths, train_hr_paths, train_lr_reg_paths):
        ...
        self.files_lr_reg = train_lr_reg_paths
        ...
        assert (len(self.files_lr) == len(self.files_lr_reg))
    ...
    def __getitem__(self, index):
        ...
        img_lr_reg = cv2.imread(self.files_lr_reg[index % len(self.files_lr_reg)])  # Should be BGR
        ...
        if random.random() < opt["augment_prob"]:
            img_lr, img_lr_reg, img_degrade_hr, img_hr = self.augment([img_lr, img_lr_reg, img_degrade_hr, img_hr])

        # Transform to Tensor
        img_lr = self.transform(img_lr)
        img_lr_reg = self.transform(img_lr_reg)
        img_degrade_hr = self.transform(img_degrade_hr)
        img_hr = self.transform(img_hr)  # ToTensor() is already in the range [0, 1]
        # operate to satisfy Simple-Align

        return {"lr": img_lr, "degrade_hr": img_degrade_hr, "hr": img_hr, "lr_reg": img_lr_reg}
