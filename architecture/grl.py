...
class GRL(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = bchw_to_blc(x)
        x = self.norm_start(x)
        x = self.pos_drop(x)        # SimpleAlign应该是不dropout的
    ...
    def forward(self, x):
        ...
        # print(f"net_g_out1.shape===>{net_g_out1.shape}")
        # print(f"net_g_out2.shape===>{net_g_out2.shape}")
        # print(f"net_g_out3.shape===>{net_g_out3.shape}")
        return x[:, :, : H * self.upscale, : W * self.upscale], net_g_out1, net_g_out2, net_g_out3
