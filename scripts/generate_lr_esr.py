...
def crop_process(path, crop_size, lr_dataset_path, output_index):
    ...
    for (h, w) in res_store:
        cropped_img = img[h + shift_offset_h: h + crop_size + shift_offset_h,
                      w + shift_offset_w: w + crop_size + shift_offset_w, ...]
        cropped_img = np.ascontiguousarray(cropped_img)
        cv2.imwrite(osp.join(lr_dataset_path, f'img_{output_index:06d}.png'), cropped_img,
                    [cv2.IMWRITE_PNG_COMPRESSION, 0])  # Save in lossless mode
...
if __name__ == '__main__':
    ...
    parser.add_argument('--flag', type=str, help='Input folder')
    ...
    print(args.flag)
    if args.flag == 'no':
        args.output = opt["lr_dataset_path"]
    else:
        args.output = opt["lr_dataset_path_reg"]
    print(args.output)
    main(args)
