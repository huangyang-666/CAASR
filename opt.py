...
opt['architecture'] = "GRLGAN"                # "ESRNET" || "ESRGAN" || "GRL" || "GRLGAN" (GRL only support 4x) || "DAT" || "DATGAN"
...
opt["lr_dataset_path_reg"] = "datasets/train_lr_reg"    # Where you temporally store the LR synthetic images
opt['hr_size'] = 256
...
opt['train_dataloader_workers'] = 16      # Number of workers for DataLoader
opt['checkpoints_freq'] = 5             # frequency to store checkpoints in the folder (unit: epoch)
...
if opt['architecture'] == "GRL":             # L1 loss training version
    ...
    opt['train_iterations'] = 210000         # Training Iterations
    opt['train_batch_size'] = 48             # 4x: 32 (256x256); 2x:  4?
    
    # Learning Rate
    opt["start_learning_rate"] = 0.0002      # Training Epoch, use the as Real-ESRGAN: 0.0001 - 0.0002 is ok, based on your need
    opt['decay_iteration'] = 70000          # Decay iteration
    ...
    opt['train_batch_size'] = 28             # 4x: 32 batch size (for 256x256); 2x: 4
...
# Parallel Process
opt['parallel_num'] = 16     # Multi-Processing num; Recommend 6~8 based on your CPU
...
# color change -->ourself
opt["color_shift"] = [0, 10]  # Harder JPEG compression setting
opt["color_banding"] = [1, 10]  # Harder JPEG compression setting
opt["color_blocking"] = [0.2, 1]  # Harder JPEG compression setting
opt["saturation_scale"] = [0.95, 1.05]  # Harder JPEG compression setting
opt["contrast_scale"] = [0.95, 1.05]  # Harder JPEG compression setting
